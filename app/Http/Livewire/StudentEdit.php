...
class StudentEdit extends Component
{
    ...
    public function mount($studentId)
        ...
        //         $grantees = $this->student->grantees->all();  // Retrieve all grantees at once
        ...
        //         $this->school_year = []; // or provide a default value
        //         $this->semester = [];
        //         $this->selectedScholarshipType1 = [];
        //         $this->selectedfundsources1 = [];
        //         $this->selectedScholarshipType2 = [];
        //         $this->selectedfundsources2 = [];
        //     }  
    ...
    public function studentSearch()
    {
        ...
        if (!$this->existingStudent) {
            ...
            if ($this->existingStudent->grantees->isNotEmpty()) {
                $grantees = $this->existingStudent->grantees->all();  // Retrieve all grantees at once
    ...
    $fundSources1 = ScholarshipName::where('scholarship_type', $this->selectedScholarshipType1)->where('status', 0)->get();
    $fundSources2 = ScholarshipName::where('scholarship_type', $this->selectedScholarshipType2)->where('status', 0)->get();
    ...
    public function addScholarship()
    {
        ...
        // count the existing
        $existingGranteeCount = Grantee::where('student_id', $this->student->student_id)
            ->whereNotIn('id', $this->scholarshipsToCreate->pluck('id'))
            ->count();
    
        // Create or update Grantee records for each scholarship, only if count is below 2
        $scholarshipsCreatedCount = 0;
        foreach ($scholarshipsToCreate as $scholarship) {
            if ($existingGranteeCount < 2) {
                // Check for existing record with matching student_id, school_year, and semester
                $granteeExists = Grantee::where([
                    'student_id' => $this->student->student_id,
                ])->exists();

                // Create a new Grantee record if it doesn't exist yet
                if (!$granteeExists) {
                    Grantee::create([
                        'student_id' => $this->student->student_id,
                ...
                // Handle the case where the student already has 2 records (e.g., display a message)
                session()->flash('error', 'The student already has the maximum allowed number of scholarships.');
            }
        }
